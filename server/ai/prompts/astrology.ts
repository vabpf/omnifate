export function buildAstrologyPrompt(
  ...
  astroData: { sunSign: string; moonSign: string; ascendant: string; planets: any[]; aspects: any[] }
): string {
...
Dữ liệu chiêm tinh đã tính:
- Cung Mặt Trời: ${astroData.sunSign}
- Cung Mặt Trăng: ${astroData.moonSign}
- Cung Mọc: ${astroData.ascendant}
- Các hành tinh: ${JSON.stringify(astroData.planets.map(p => `${p.name} ở ${p.sign} (${p.degree}°, nhà ${p.house})`))}
- Các góc chiếu: ${JSON.stringify(astroData.aspects.map(a => `${a.planet1} - ${a.planet2}: ${a.type} (${a.angle}°)`))}
...
4. 🌍 **Tổng hợp Bản đồ Sao**: Phân tích tương tác giữa các hành tinh, các góc chiếu chính và ý nghĩa tổng thể đối với cuộc đời.
