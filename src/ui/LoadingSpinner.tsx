...
const colorMap: Record<AccentColor, { ping: string; spin: string; text: string; gradient: string }> = {
  amber:   { ping: 'border-warm-amber/20', spin: 'border-t-warm-amber', text: 'text-warm-amber', gradient: 'from-warm-amber to-warm-teal' },
  teal:    { ping: 'border-warm-teal/20',  spin: 'border-t-warm-teal',  text: 'text-warm-teal',  gradient: 'from-warm-teal to-warm-amber' },
  ...
  slate:   { ping: 'border-white/10',      spin: 'border-t-white/40',   text: 'text-white/50',   gradient: 'from-white/10 to-white/5' },
};
      ...
      <div className="fixed inset-0 bg-[#0a0a0a]/90 backdrop-blur-2xl z-50 flex flex-col items-center justify-center p-6 select-none animate-fade-in border border-white/5">
        <div className="relative w-24 h-24 mb-8">
          ...
          <div className={`absolute inset-3 rounded-full bg-gradient-to-tr flex items-center justify-center shadow-lg shadow-warm-amber/30 ${colors.gradient}`}>
            <Sparkles className="w-8 h-8 text-warm-amber animate-pulse" />
          </div>
        </div>
        <div className="max-w-md text-center space-y-3">
          ...
          <p className="text-sm text-white/60 min-h-[40px] font-medium leading-relaxed italic animate-pulse-slow">"{text}"</p>
