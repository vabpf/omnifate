      ...
      <div className="glass-card rounded-2xl p-6 hover:translate-y-0">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-gradient-to-b from-indigo-950/40 to-black/30 rounded-xl p-4 border-2 border-indigo-400 text-center space-y-3 relative overflow-hidden shadow-lg shadow-indigo-500/10 hover:border-indigo-300 transition duration-300">
            <div className="absolute top-0 right-0 bg-indigo-500 text-[8px] text-slate-100 font-bold px-1.5 py-0.5 rounded-bl">NHẬT NGUYÊN</div>
          ...
          {aiInterpretation ? (
            <div className="mt-6 pt-4 border-t border-white/10 space-y-4">
              <h5 className="font-display text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-400 flex items-center gap-1.5">
                <Star className="w-4 h-4 text-purple-400" />Đại Sư Luận Giải Bát Tự
              </h5>
              <div className="space-y-4 text-xs leading-relaxed text-slate-300 max-h-[170px] overflow-y-auto pr-1">
                <div><h6 className="font-bold text-slate-200 mb-0.5">Phân Tích Thừa Thiếu Ngũ Hành:</h6><div className="p-3 bg-black/40 rounded-xl border border-white/5"><MarkdownRenderer content={aiInterpretation.elementAnalysis} theme="amber" /></div></div>
                <div><h6 className="font-bold text-slate-200 mb-0.5">Dụng Thần / Hỷ Thần trợ giúp (Vũ khí cát tinh):</h6><div className="p-3 bg-black/40 rounded-xl border border-white/5"><MarkdownRenderer content={aiInterpretation.favourableElements} theme="emerald" /></div></div>
                <div><h6 className="font-bold text-slate-200 mb-0.5">Kỵ Thần tránh né (Thế lực xung sát):</h6><div className="p-3 bg-black/40 rounded-xl border border-white/5"><MarkdownRenderer content={aiInterpretation.unfavourableElements} theme="rose" /></div></div>
                <div><h6 className="font-bold text-slate-200 mb-0.5">Phương Pháp Cải Vận Trực Tiếp:</h6><div className="p-3 bg-black/40 rounded-xl border border-white/5"><MarkdownRenderer content={aiInterpretation.advice} theme="purple" /></div></div>
              </div>
            </div>
          ) : (
            <div className="mt-6 bg-black/20 p-4 rounded-xl border border-white/5 text-xs text-slate-400 italic text-center">Vận hành tính năng "Xem kết quả đại sư" kích hoạt gói tinh vân AI để nhận phân tích Dụng Thần cải vận đặc biệt.</div>
          )}
        </div>
      </div>
    </div>
