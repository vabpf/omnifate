          ...
          <div className="w-full flex items-center justify-between mb-4">
            <div>
              <h4 className="font-display text-base font-bold text-slate-200 flex items-center gap-2">
                <Compass className="w-5 h-5 text-indigo-400" />Vòng Tròn Bản Đồ Sao (Natal Chart Wheel)
              </h4>
            ...
            {selectedPlanet ? (
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-[10px] font-mono text-slate-300 bg-white/5 px-2 py-0.5 rounded border border-white/10">Cung {selectedPlanet.house} (Góc {selectedPlanet.degree}°)</span>
                </div>
                ...
                <p className="text-xs text-slate-300 mt-2 leading-relaxed">Thể hiện nguồn lực lớn về phương diện {selectedPlanet.name.toLowerCase()} giúp điều hướng phong vĩ, biểu đạt cá tính hoàn mỹ trong tinh vân sự nghiệp.</p>
              </div>
            ) : (
              <div className="text-center py-6 text-xs text-slate-500 italic">Nhấp vào bất kỳ điểm chấm hành tinh nào trong Bản đồ sao ở trên để giải mã tọa độ.</div>
            )}
          </div>
        </div>
          ...
          {aiInterpretation && (
            <div className="glass-card rounded-2xl p-6 space-y-4 hover:translate-y-0">
              <h4 className="font-display text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-400 flex items-center gap-1.5">
                <Star className="w-5 h-5 text-indigo-400 fill-indigo-400/10" />Đại Sư Luận Giải Bản Đồ Sao
              </h4>
              <div className="space-y-4 text-xs leading-relaxed text-slate-300 overflow-y-auto max-h-[300px] pr-1">
                <div><h5 className="font-bold text-slate-200 mb-1">Cung Mặt Trời ({data.sunSign}) - Tôi Luôn Cố Gắng:</h5><div className="p-3 bg-black/40 rounded-xl border border-white/5"><MarkdownRenderer content={aiInterpretation.sunSignInterpretation} theme="indigo" /></div></div>
                <div><h5 className="font-bold text-slate-200 mb-1">Cung Mặt Trăng ({data.moonSign}) - Cảm Xúc Sâu Kín:</h5><div className="p-3 bg-black/40 rounded-xl border border-white/5"><MarkdownRenderer content={aiInterpretation.moonSignInterpretation} theme="indigo" /></div></div>
                <div><h5 className="font-bold text-slate-200 mb-1">Cung Mọc ({data.ascendant}) - Lớp Vỏ Xã Hội:</h5><div className="p-3 bg-black/40 rounded-xl border border-white/5"><MarkdownRenderer content={aiInterpretation.ascendantInterpretation} theme="indigo" /></div></div>
                <div><h5 className="font-bold text-slate-200 mb-1">Hợp Nhất Toàn Diện Bản Đồ Sao:</h5><div className="p-3 bg-black/40 rounded-xl border border-white/5"><MarkdownRenderer content={aiInterpretation.natalChartSynthesis} theme="indigo" /></div></div>
              </div>
            </div>
          )}
        </div>
