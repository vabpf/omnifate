...
export default function MarkdownRenderer({ content, theme = 'purple', className = '' }: MarkdownRendererProps) {
  // Theme map to define colors for h1, h2, h3, h4, strong, blockquote
  ...
  const components = {
    h1: ({ children }: any) => (
      <h1 className={`text-sm font-bold mt-4 mb-2 font-display uppercase tracking-wider border-b pb-1 ${style.h1}`}>
        {children}
      </h1>
    ),
    h2: ({ children }: any) => (
      <h2 className={`text-xs font-bold mt-3.5 mb-1.5 font-display uppercase tracking-wide ${style.h2}`}>
        {children}
      </h2>
    ),
    h3: ({ children }: any) => (
      <h3 className={`text-[11px] font-bold mt-3 mb-1 font-mono uppercase ${style.h3}`}>
        {children}
      </h3>
    ),
    h4: ({ children }: any) => (
      <h4 className={`text-[10px] font-bold mt-2 mb-1 uppercase ${style.h4}`}>
        {children}
      </h4>
    ),
    p: ({ children }: any) => (
    ...
    ul: ({ children }: any) => (
      <ul className="list-disc pl-4 space-y-1 my-2 text-xs text-slate-300">
        {children}
      </ul>
    ),
    ol: ({ children }: any) => (
      <ol className="list-decimal pl-4 space-y-1 my-2 text-xs text-slate-300">
        {children}
      </ol>
    ),
    li: ({ children }: any) => (
    ...
    blockquote: ({ children }: any) => (
      <blockquote className={`border-l-2 pl-3 italic text-xs my-3 py-1.5 rounded-r-md ${style.blockquote}`}>
        {children}
      </blockquote>
    ),
    strong: ({ children }: any) => (
    ...
    a: ({ children, href }: any) => (
      <a href={href} target="_blank" rel="noopener noreferrer" className={`underline cursor-pointer ${style.link}`}>
        {children}
      </a>
    ),
  };
