...
export default function LandingPage() {
  const mysticalPillars = [
    {
      title: 'Thần Số Học',
      icon: <Layers className="w-5 h-5 text-purple-400" />,
      desc: 'Giải toán 29 chỉ số cốt tủy: Đường đời, sứ mệnh, khát khao linh hồn, nợ nghiệp và bài học lớn.',
      bg: 'from-purple-950/40 to-indigo-950/10 border-purple-500/10'
    },
    {
      title: 'Chiêm Tinh Học',
      icon: <Compass className="w-5 h-5 text-indigo-400" />,
      desc: 'Bản Đồ Sao hoàng đạo Tây Phương xác lập vị trí cung mọc, mặt trăng, mặt trời chiếu mệnh.',
      bg: 'from-indigo-950/40 to-blue-950/10 border-indigo-500/10'
    },
    {
      title: 'Lá Số Tử Vi',
      icon: <Award className="w-5 h-5 text-amber-400" />,
      desc: 'Đông phương lý số an bài 14 chính tinh và thần sát linh diệu trên sơ đồ 12 cung bản mệnh.',
      bg: 'from-amber-950/30 to-slate-950/10 border-amber-500/10'
    },
    {
      title: 'Bát Tự Ngũ Hành',
      icon: <Activity className="w-5 h-5 text-teal-400" />,
      desc: 'Tính toán biểu đồ ngũ hành Kim - Thủy - Mộc - Hỏa - Thổ tìm khuyết âm và hỷ dụng thần.',
      bg: 'from-teal-950/30 to-emerald-950/10 border-teal-500/10'
    },
    {
      title: 'Thiết Kế Nhân Dạng',
      icon: <FileText className="w-5 h-5 text-rose-400" />,
      desc: 'Phác thảo bản đồ Bodygraph định tính loại hình, thẩm quyền nội tại và chiến lược tương tác.',
      bg: 'from-rose-950/30 to-red-950/10 border-rose-500/10'
    }
  ];
  ...
  return (
    <div className="min-h-screen text-slate-100 flex flex-col relative overflow-hidden bg-[#050614]">
      ...
      {/* HERO & AUTHENTICATION MAIN CONTENT AREA */}
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20 flex flex-col lg:grid lg:grid-cols-12 gap-12 lg:gap-8 items-center relative z-10">
        ...
        {/* Left Side: Elegant value proposition */}
        <div className="lg:col-span-7 space-y-8 text-center lg:text-left">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-gradient-to-r from-purple-950/80 to-indigo-950/80 border border-purple-500/20 rounded-full text-xs font-mono text-purple-300 shadow-inner">
              ...
              <span>Hợp Nhất Tri Thức Mệnh Lý Đông - Tây</span>
            </div>
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-display font-extrabold tracking-tight leading-[1.1] text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-100 to-indigo-200">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-purple-400 to-indigo-400">
                Kiến Tạo Hành Trình
              </span>
            </h1>
            
            <p className="text-sm sm:text-base text-slate-300 max-w-xl mx-auto lg:mx-0 leading-relaxed">
              Trải nghiệm ứng dụng tối thượng tích hợp 5 hệ thống huyền học cổ truyền và hiện đại. Nhận bản báo cáo Thần số học 29 mục chuyên sâu cùng sự đồng hành của Trí tuệ nhân tạo Gemini.
            </p>
          </div>

          {/* Graphical display grid of the 5 mystical arts */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto lg:mx-0 text-left">
            {mysticalPillars.map((pillar, i) => (
              <motion.div
                key={pillar.title}
        ...
        {/* Right Side: Ultimate auth login/signup container block */}
        <div className="lg:col-span-5 w-full max-w-md mx-auto">
          ...
          >
            {/* Top golden glowing ambient lighting */}
