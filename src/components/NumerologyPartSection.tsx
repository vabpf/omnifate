...
interface NumerologyPartSectionProps {
  partId: 'A' | 'B' | 'C' | 'D';
  title: string;
  description: string;
  loadingText: string;
...
export default function NumerologyPartSection({
  partId,
  title,
  description,
  loadingText,
  ...
  content,
  onGenerate,
}: NumerologyPartSectionProps) {
  if (isLoading) {
    return <LoadingState text={loadingText} accentColor="purple" />;
  }
  ...
  if (error) {
    return (
      <ErrorState 
        message={error} 
        onRetry={onGenerate} 
        retryText={`Thử Lại Luận Giải Phần ${partId}`} 
      />
    );
  }
